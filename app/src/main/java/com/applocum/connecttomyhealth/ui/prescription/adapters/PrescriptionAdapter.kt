...
class PrescriptionAdapter(
    ...
    private val onPrescriptionClick: PrescriptionClickListner
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        when (viewType) {
            ITEM -> {
                ...
                viewHolder = PrescriptionHolder(viewItem)
            }
            LOADING -> {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (getItemViewType(position)) {
            ITEM -> {
                val prescriptionHolder = holder as PrescriptionHolder
                prescriptionHolder.itemView.tvDocType.text =
                    mContext.resources.getString(R.string.prescription)
                prescriptionHolder.itemView.tvDate.text = document?.created_at?.let {
                    convertDocumentTime(
                        it
                    )
                }
                prescriptionHolder.itemView.tvDoctorName.text = ("By" + " " + document?.by)

                RxView.clicks(prescriptionHolder.itemView).throttleFirst(500, TimeUnit.MILLISECONDS)
                    .subscribe {
                        document?.let { it1 ->
